...
<div class="list">
  ...
  <!-- When we remove below header slot, the FilterableList component will try to render the header slot content.
       ...we're left with an ugly double border in UI because the following code is still rendering the <div class="header">.
       We can fix that by using the special $$slots variable like below:  -->
  {#if $$slots.header}
    <div class="header">
      <slot name="header" />
    </div>
