<script lang="ts">
  ...
  _$logger.debug.TraceLayoutAndPage('/sveltekit/streaming/+page.svelte', $page.route.id, data);
...
<main>
  <div>
    ...
    <span>
      ...
      {:then delayedData}
        <ProgressRadial {value} stroke={100} meter="stroke-primary-500" track="stroke-primary-500/30">{delayedData}</ProgressRadial>
      {:catch error}
        {error.message}
      {/await}
    </span>
