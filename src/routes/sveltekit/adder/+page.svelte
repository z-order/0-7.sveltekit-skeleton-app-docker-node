<script>
  ...
  async function add() {
    const response = await fetch('/sveltekit/api/adder', {
      method: 'POST',
...
<style lang="postcss">
  main {
    @apply h-[50%] flex flex-row gap-8 justify-center items-center;
  }
  form {
    @apply flex flex-row gap-8 justify-center items-center space-x-1;
  }
  input {
